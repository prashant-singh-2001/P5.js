...
function disable_buttons() {
  ...
  for (let i = 0; i < input.length; i++) input[i].disabled = true;
  return parseInt(document.getElementById("delay").value);
}

function enable_buttons() {
  let bar = document.getElementsByClassName("bar");
  for (let i = 0; i < bar.length; i++) bar[i].style.backgroundColor = "#33ff00";
  bar = document.getElementsByTagName("input");
  for (let i = 0; i < bar.length; i++) bar[i].disabled = false;
}
